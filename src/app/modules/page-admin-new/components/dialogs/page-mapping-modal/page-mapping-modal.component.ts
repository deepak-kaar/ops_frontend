...
export class PageMappingModalComponent implements OnInit {
  ...
  /** Dropdown options for choosing Entity vs Instance (and Tags). Stored in form as sourceType. */
  sourceTypeOptions = [
        ...
        for (const [propertyName, propertyDef] of Object.entries(schemaValue as any)) {
          ...
          if (this.mode === 'edit') {
            ...
            if (propDef.mapping && propDef.mapping.type !== null) {
              ...
              existingMapping = { ...existingMapping, ...propDef.mapping };
            } else {
          ...
          const propertyGroup = this.fb.group({
            ...
            sourceType: [existingMapping.sourceType || null], // Entity | Instance | Tags
          ...
          // Subscribe to source type (Entity/Instance/Tags) changes – clear entity/instance selection
          propertyGroup.get('sourceType')?.valueChanges.subscribe(() => {
          ...
          if (this.mode === 'edit' && existingMapping.type === 'attribute' && existingMapping.entityOrInstanceId) {
            ...
            if (entityOrInstance) {
              // Restore or infer sourceType for edit/legacy mappings
              const sourceType = existingMapping.sourceType || entityOrInstance.sourceType;
              if (sourceType) {
                propertyGroup.get('sourceType')?.setValue(sourceType, { emitEvent: false });
              }
              propertyGroup.get('entityOrInstanceId')?.setValue(existingMapping.entityOrInstanceId, { emitEvent: false });
  ...
  onMappingTypeChange(propertyKey: string, mappingType: string): void {
    ...
    if (mappingType === 'attribute') {
      ...
      propertyGroup.get('sourceType')?.setValue(null);
      propertyGroup.get('attributeId')?.setValue(null);
      propertyGroup.get('attributeName')?.setValue(null);
      propertyGroup.get('entityOrInstanceId')?.setValue(null);
      propertyGroup.get('entityOrInstanceName')?.setValue(null);
  ...
  /**
   * Gets entity/instance options based on mapping type and selected sourceType (Entity | Instance | Tags).
   */
  getEntityOrInstanceOptions(propertyKey: string, mappingType: string, sourceType: string | null): any[] {
    if (mappingType !== 'attribute') return [];
    const propertyGroup = this.findPropertyGroup(propertyKey);
    if (!propertyGroup) return [];

    if (sourceType === 'Entity') {
      return this.entityList.map(e => ({ ...e, sourceType: 'Entity' }));
    }
    if (sourceType === 'Instance') {
      return this.instanceList.map(i => ({ ...i, sourceType: 'Instance' }));
    }
    if (sourceType === 'Tags') {
      return this.tagsList.map(t => ({ ...t, sourceType: 'Tags' }));
    }
    return [];
  }
