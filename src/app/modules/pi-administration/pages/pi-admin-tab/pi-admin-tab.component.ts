...
import { ConfirmationDailogExcelComponent } from '../../components/confirmation-dailog-excel/confirmation-dailog-excel.component';
  ...
  processImportData(groupedChanges: any): void {
    ...
    groupedChanges.addNew.forEach((item: ExcelImportResult) => {
      ...
      const apiCall = isSendTab
        ? this.piAdminService.postPISend(payload)
        : this.piAdminService.postPIReceive(payload);
    ...
    groupedChanges.edit.forEach((item: ExcelImportResult) => {
      ...
      const apiCall = isSendTab
        ? this.piAdminService.putPISend(payload, id)
        : this.piAdminService.putPIReceive(payload, id);
    ...
    groupedChanges.delete.forEach((item: ExcelImportResult) => {
      ...
      const apiCall = isSendTab
        ? this.piAdminService.deletePISend(id)
        : this.piAdminService.deletePIReceive(id);
