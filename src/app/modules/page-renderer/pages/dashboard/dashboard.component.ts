...
export class DashboardComponent extends PageRendererComponent implements OnInit, OnDestroy {
  ...
  /**
   * Loads templates from the service - uses 'Dashboard Design' templateType
   */
  loadMappings() {
    this.loading = true;
    ...
    this.pageRendererService.getTemplates(payload).subscribe({
      next: (res: any) => {
        ...
        // Preserve selection if possible; else auto-select first item
      ...
      error: (err) => {
        console.error('Error loading dashboard templates:', err);
        this.loading = false;
      }
    });
  }
  ...
  applyListFilter() {
    ...
    this.displayMappings = this.orderedMappings.filter(m => {
      const name = (m.mappingName || m.name || m.templateName || '').toString().toLowerCase();
      const desc = (m.mappingDescription || m.description || m.templateDescription || '').toString().toLowerCase();
