...
export class EmailTableComponent extends EmailAdministrationComponent implements OnInit, OnDestroy {
 ...
 */
  ...
  editEmail(email_data: any) {
    this.mode = "edit";
    this.editingEmail = email_data;
    this.isShowDetails = true;
  }
 ...
 */
  ...
  deleteEmail(data: any) {
    ...
    deleteDialogRef.onClose.subscribe((result: any) => {
      if (result?.confirmed) {
        this.emailAdministrationService.deleteEmail(data.emailId).subscribe({
          next: () => {
