...
import { BehaviorSubject, catchError, combineLatest, distinctUntilChanged, finalize, forkJoin, map, Observable, of, startWith, Subject, takeUntil } from 'rxjs';
...
export class WebserviceTableComponent extends WebserviceAdministrationComponent implements OnInit {
  ...
  private rawWsData$!: Observable<any[]>;

  selectedWebService: any[] = [];
  activeWS: any;
  
  // Table configuration
  columns: TableColumn[] = [
    { field: 'webserviceId', header: 'Webservice ID', sortable: true, filterable: false, minWidth: '10rem' },
    { field: 'wsName', header: 'Webservice Name', sortable: true, filterable: false, minWidth: '12rem' },
    { field: 'description', header: 'Description', sortable: true, filterable: false, minWidth: '14rem' },
    { field: 'active', header: 'Status', sortable: true, filterable: false, minWidth: '8rem', template: 'custom' }
  ];
  
  actions: TableAction[] = [
    { icon: 'pi pi-cloud-download', tooltip: 'Fetch', severity: 'success', action: 'fetch' },
  ...
  tableConfig: TableConfig = {
    ...
    paginator: true,
    globalFilterFields: ['webserviceId', 'wsName', 'description'],
    selectionMode: 'multiple',
    rowHover: true,
    emptyMessage: 'No Web Service found.',
    showCaption: false,
    showSearch: false,
  ...
  getWS(): void {
    ...
    this.rawWsData$ = this.ws_data$;
    this.ws_data$ = combineLatest([
      this.rawWsData$,
      this.search$.pipe(startWith(this.searchWSValue), distinctUntilChanged())
    ]).pipe(
      map(([data, term]) => this.filterWSData(data || [], term))
    );
  }
  ...
  onWSSelect(event: any): void {
    this.activeWS = event.data;
    this.isShowMappingTable = true;
  }

  onRowSelect(event: any): void {
    this.selectedWebService = event.data ? [event.data] : [];
  }
  ...
  onSearchChange(value: string): void {
    this.searchWSValue = value;
    this.search$.next(value || '');
  }

  private filterWSData(data: any[], term: string): any[] {
  ...
  exportExcel() {
    this.ws_data$.subscribe(data => {
      const exportData = this.selectedWebService && this.selectedWebService.length > 0 ? this.selectedWebService : data || [];
